...
#define LWVRAYFUNCS_GLOBAL	"VRayFuncs"

typedef struct st_VRayFuncs
{
 ...
 */
typedef struct st_VolumetricEvaluationFuncs
{
	...
	void (*init)( unsigned long slot, int LWINIT_context);

	/// Syncronize the volumetric handlers with current Layout time.
	/// pass slot of 0 to process all volumetric servers, otherwise pass specific 1-based slot index of desired server
	void (*newtime)( unsigned long slot);

	/// send the ray through to each volumetric handler for evaluation
	/// pass slot of 0 to process all volumetric servers, otherwise pass specific 1-based slot index of desired server
	double (*evaluate)( unsigned long slot, LWVolumeAccess *ray, unsigned short LWVOLF_raytype);

	/// Cleanup the volumetric handlers when done being evaluated.
	/// pass slot of 0 to process all volumetric servers, otherwise pass specific 1-based slot index of desired server
	void (*cleanup)( unsigned long slot);

} VolumetricEvaluationFuncs;
